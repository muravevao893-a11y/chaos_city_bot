...
По умолчанию реальные покупки выключены:

```env
STARS_ENABLED=false
```

Чтобы включить оплату Stars:

```env
STARS_ENABLED=true
```

Для Telegram Stars используется валюта `XTR` и пустой `provider_token`, как требуется для цифровых товаров в Bot API. Покупки в v1.4 не дают боевой силы: только косметика, AI-газета, премиум-события и статус.

Команды:

```text
/stars       - лавка Stars
/titles      - рынок титулов за игровые монеты
/buytitle    - купить титул по ключу
```
...
```text
/topweek        - топ городов недели
/hall           - зал славы
/feedback текст - оставить отзыв
/admin          - админ-центр проекта
/admin_chats    - список активных чатов
