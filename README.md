...
- Добавлены **фракции** внутри города: работяги, мафия, банкиры, пресса, законники и бунтари.
- Добавлены **предметы**: ключ от подвала, иммунитет суда, компромат, дымовая шашка, фальшивая корона.
- Добавлены **ограбления казны** с шансом успеха, штрафами и подвалом.
- Добавлены **бунты против власти** как отдельное голосование.
- Добавлен **побег из подвала**.
- Добавлена **летопись города**: важные события теперь сохраняются в истории района.
- Добавлены редкие **легендарные события** после значимых действий.
- Улучшен профиль: теперь показывает фракцию и количество предметов.
- Оптимизация БД: новые индексы для фракций, краж, логов действий и дуэлей; JSON-данные ограничены и чистятся перед записью.

## Что было в v0.8.1

- Inline-сообщения теперь редактируются на месте через `edit_message_text`: голосования, события, панели и меню не удаляются и не отправляются заново.
- Удаление старого callback-сообщения заменено на безопасный no-op: чат меньше мигает и меньше спамится.
- Автосообщения тоже сначала пытаются обновить последнюю панель, а не плодить новые сообщения.
- Если Telegram не разрешит редактирование, бот fallback-ом отправит новое managed-сообщение и запомнит его.

## Что было добавлено в v0.8
