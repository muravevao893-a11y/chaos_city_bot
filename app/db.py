...
def _run_light_migrations() -> None:
    ...
    _add_column_if_missing("cities", "history_json", "history_json TEXT NOT NULL DEFAULT '[]'")
    _add_column_if_missing("cities", "premium_json", "premium_json TEXT NOT NULL DEFAULT '{}'")
    ...
    _add_column_if_missing("cities", "activity_mode", "activity_mode VARCHAR(20) NOT NULL DEFAULT 'normal'")
    _add_column_if_missing("cities", "last_daily_summary_at", f"last_daily_summary_at {_timestamp_ddl()}")
    ...
    _add_column_if_missing("memberships", "faction", "faction VARCHAR(32)")
    _add_column_if_missing("memberships", "inventory_json", "inventory_json TEXT NOT NULL DEFAULT '{}'")
    ...
    _add_column_if_missing("memberships", "achievements_json", "achievements_json TEXT NOT NULL DEFAULT '[]'")
    _add_column_if_missing("memberships", "secret_role", "secret_role VARCHAR(32)")
    _add_column_if_missing("memberships", "mission_key", "mission_key VARCHAR(32)")
    _add_column_if_missing("memberships", "mission_started_at", f"mission_started_at {_timestamp_ddl()}")
    _add_column_if_missing("memberships", "mission_completed_at", f"mission_completed_at {_timestamp_ddl()}")
    _add_column_if_missing("memberships", "last_steal_at", f"last_steal_at {_timestamp_ddl()}")
    _add_column_if_missing("memberships", "last_revolt_at", f"last_revolt_at {_timestamp_ddl()}")
    ...
    _create_index_if_missing("ix_membership_secret_role", "CREATE INDEX ix_membership_secret_role ON memberships (city_id, secret_role)")
    _create_index_if_missing("ix_membership_mission", "CREATE INDEX ix_membership_mission ON memberships (city_id, mission_key, mission_completed_at)")
